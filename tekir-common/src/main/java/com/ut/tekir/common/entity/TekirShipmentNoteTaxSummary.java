...
@Entity @Table(name = "TEKIR_SHIPMENT_NOTE_TAX_SUMMARY") @Getter @Setter @EqualsAndHashCode(of = "id", callSuper = false)
public class TekirShipmentNoteTaxSummary extends AuditBase {
    private static final long serialVersionUID = 1L;
    @Id @GeneratedValue(strategy = GenerationType.TABLE, generator = "genericSeq") @Column(name = "ID") private Long id;
    @ManyToOne @JoinColumn(name = "OWNER_ID") private TekirShipmentNote owner;
    @ManyToOne @JoinColumn(name = "TAX_ID") private Tax tax;
